...
//#define int int32_t
  ...
  for(int i = 0; i < out.size(); i ++) {
    ...
    if(! savep2or3<num_t>((std::string("predg-") + std::to_string(i) + std::string(".ppm")).c_str(), normalize<num_t>(autoLevel<num_t>(outs, int(num_t(outs.size() * outs[0].rows() * outs[0].cols()) * num_t(int(1)) / num_t(int(3)) ) )) ))
