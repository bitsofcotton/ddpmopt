...
#define int int32_t
  ...
  if(len < 0) {
    ...
    vector<num_t> intensity;
    work.reserve(size);
    intensity.reserve(size);
    for(int j = 0; j < size; j ++) {
      SimpleVector<num_t> vwork;
      std::cin >> vwork;
      intensity.emplace_back(vwork[vwork.size() - 2]);
    ...
    while(std::getline(std::cin, s, '\n')) {
      ...
      for(int j = 0; j < abs(len); j ++) {
        ...
        s += char(int(revertProgramInvariant<num_t>(make_pair(work[Midx].dot(pinv.first) * intensity[Midx], num_t(int(1)) /* pinv.second */)) * num_t(int(256)) ));
      }
      std::cout << s << std::endl;
    }
  } else if(0 <= len) {
